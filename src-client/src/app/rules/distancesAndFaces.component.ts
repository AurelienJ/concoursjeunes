  ...
  template: `<div class="modal modal-primary" id="confirmDeleteDistanceAndFacesSetItemModal" *ngIf="selectedDistanceAndFacesSetForDelete">
        <div class="modal-dialog">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Fermer"
                  (click)="selectedRankingCriterionForDelete=null">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-outline"
                  (click)="deleteDistanceAndFacesSet(selectedDistanceAndFacesSetForDelete, true)">Supprimer</button>
                <button type="button" class="btn btn-outline" data-dismiss="modal"
                  (click)="selectedDistanceAndFacesSetForDelete=null">Fermer</button>
            </div>
            </div>
            <!-- /.modal-content -->
        </div>
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <a href="javascript:void(0)" class="btn btn-app" (click)="addDistanceAndFacesSet()">
                ...
                <div class="col-sm-12">
                    <ul class="list-group" id="criteria-collection">
                        <li class="list-group-item clearfix" *ngFor="let distanceAndFaces of distancesAndFacesSet">
                            <a href="javascript:void(0)" class="button-align"
                                (click)="selectedDistanceAndFacesSet = distanceAndFaces">
                              {{distanceAndFaces.name || '<Nouveau jeux de distances / blasons>'}}</a>
                            <a href="javascript:void(0)" class="pull-right btn btn-link"
                              (click)="deleteDistanceAndFacesSet(distanceAndFaces)"><i class="fa fa-trash" title="Supprimer"></i></a>
                        </li>
                    </ul>
                </div>
            </div>
...
export class DistancesAndFacesComponent implements OnInit {
  ...
  @Input() public nbSerie: number;

  @Output()
  public distancesAndFacesSetChange: EventEmitter<
    IDistanceAndFacesSet[]
  > = new EventEmitter<IDistanceAndFacesSet[]>();
  ...
  public addDistanceAndFacesSet() {
    const distanceAndFaces: IDistanceAndFaces[] = [];
    ...
    const distanceAndFacesSet = <IDistanceAndFacesSet>{
      tempId: new Date().getTime().toString(),
      name: null,
      distancesAndFaces: distanceAndFaces
    };
    if (!this.distancesAndFacesSet) this.distancesAndFacesSet = [];
  ...
  public deleteDistanceAndFacesSet(
    distanceAndFacesSet: IDistanceAndFacesSet,
