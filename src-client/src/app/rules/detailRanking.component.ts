...
@Component({
  ...
  template: `<div *ngIf="rankingCriterion">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Fermer"
                          (click)="displayAddDiscriminantCriteriaSet=false">
                            ...
                            <div class="radio" *ngFor="let criterionElement of criterion.criterionElements">
                                <label>
                                    <input type="radio" name="criterion-{{criterion.id}}"
                                        ...
                                        [ngModel]="selectedCriterionElements.get(criterion)"
                                          (ngModelChange)="selectedCriterionElements.set(criterion, $event)"/>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-default" data-dismiss="modal"
                          (click)="displayAddDiscriminantCriteriaSet=false">Fermer</button>
                    </div>
                    <!-- /.modal-content -->
                </div>
                <!-- /.modal-dialog -->
        ...
        <div class="form-group">
            ...
            <input placeholder="Libelle" id="libelleRankingCriterion" name="libelleRankingCriterion" class="form-control"
              [(ngModel)]="rankingCriterion.name"/>
        </div>
        ...
        <div class="form-group">
            <label for="discriminantCriteria">Critères discriminants <button class="btn btn-link btn-lg"
              (click)="displayDiscriminantCriterionSet(null)"><i class="fa fa-plus-circle" aria-hidden="true"></i> Ajouter</button></label>
            <div class="form-control-static">
                ...
                <ul class="list-group clearfix">
                    ...
                    <a href="javascript:void(0)" class="pull-right btn btn-link"
                      (click)="deleteDiscriminantCriterionSet(discriminantCriterionSet)"><i class="fa fa-trash" title="Supprimer"></i></a>
                    </li>
                </ul>
            </div>
        </div>
...
export class DetailRankingComponent implements OnInit, OnChanges {
  @Input() public rankingCriterion: IRankingCriterion;

  @Input() public distanceAndFacesSets: IDistanceAndFacesSet[];

  @Input() public idFederation: string;

  public criteria: Criterion[];

  public displayAddDiscriminantCriteriaSet = false;
  ...
  ngOnChanges(changes: SimpleChanges) {
    for (const propName in changes) {
      if (propName == 'idFederation') {
        ...
        this.entitesService
          .getCriteria(this.idFederation)
          .then(c => (this.criteria = c));
      } else if (propName == 'rankingCriterion') {
        if (
          !this.rankingCriterion.distancesAndFacesSet &&
          this.rankingCriterion.idDistancesAndFacesSet
        ) {
          this.rankingCriterion.distancesAndFacesSet = _.find(
            this.distanceAndFacesSets,
            df => df.id == this.rankingCriterion.idDistancesAndFacesSet
          );
        }
      }
    }
  ...
  private getCriterionElement(idCriterionElement: string): any {
    for (let i = 0; i < this.criteria.length; i++) {
      ...
      const element = _.find(
        criterion.criterionElements,
        e => e.id == idCriterionElement
      );
      if (element) return { criterion: criterion, element: element };
    }
  }
  ...
  public validateDiscriminantCriterionSet() {
    ...
    this.selectedCriterionElements.forEach(e => {
      this.editedDiscriminantCriterionSet.elements.push(
        <IDiscriminantCriterionSetElement>{
          idCriterionElement: e.id,
          ordre: ordre++
        }
      );
      if (ordre > 1) this.editedDiscriminantCriterionSet.name += ' / ';
