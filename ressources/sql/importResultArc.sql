		...
		INNER JOIN (SELECT MAX(A.ID_CONTACT) AS MAXIDCONTACT,A.NUMLICENCEARCHER FROM ARCHERS A 
			INNER JOIN (
				SELECT MAX(DATEMODIF) AS MAXDATEMODIF, NUMLICENCEARCHER FROM ARCHERS WHERE NUMLICENCEARCHER IN (
					SELECT NUMLICENCEARCHER FROM IMPORTFFTA.ArchersFFTA)
...
UPDATE IMPORTFFTA.ArchersFFTA L SET L.ID_CONTACT=IFNULL((SELECT A.ID_CONTACT FROM ARCHERS A WHERE A.NUMLICENCEARCHER=L.NUMLICENCEARCHER LIMIT 1),L.ID_CONTACT);
...
MERGE INTO ARCHERS (ID_CONTACT,NUMLICENCEARCHER, 
    ...
    ARC, DATEMODIF) SELECT A.ID_CONTACT,NUMLICENCEARCHER, 
        CERTIFMEDICAL, GENREARCHER-1, null, 
        CATEGORIEARCHER-1, NIVEAUARCHER, ARCARCHER-1, @DATE_MODIF FROM IMPORTFFTA.ArchersFFTA A INNER JOIN CONTACT C ON A.ID_CONTACT = C.ID_CONTACT;
...
DROP TABLE IMPORTFFTA.EntiteFFTA;
DROP TABLE IMPORTFFTA.ArchersFFTA;
