...
CREATE TEMPORARY TABLE IMPORTFFTA.RESULT_CLUB AS SELECT  RANDOM_UUID() as ID_ENTITE, * from (select distinct * FROM CSVREAD('{temp}/result_club.txt',null,'UTF-8',STRINGDECODE('\t')));
...
CREATE TEMPORARY TABLE IMPORTFFTA.RESULT_LICENCE AS SELECT RANDOM_UUID() AS ID_CONTACT,* FROM CSVREAD('{temp}/result_licence.txt',null,'UTF-8',STRINGDECODE('\t'));
...
DROP TABLE IMPORTFFTA.RESULT_LICENCE;
DROP TABLE IMPORTFFTA.FFTACATEGORIE;
DROP TABLE IMPORTFFTA.FFTAARC;
DROP TABLE IMPORTFFTA.FFTANIVEAU;
